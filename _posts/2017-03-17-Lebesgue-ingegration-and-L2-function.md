...
Let m be an integer, $$(X,F)$$ be a topological space and let $$(x^{(n)})_{n=m}^{\infty}$$ be a sequence of points in $$X$$. Let $$x$$ be a point in $$X$$ *X*. We say that $$(x^{(n)})_{n=m}^{\infty}$$ *converges* to *x* if and only if, for every neighbourhood *V* of *x*, there exists an $$N\ge m$$ such that $$x^{(n)} \in V$$ for all $$n\ge N$$.
